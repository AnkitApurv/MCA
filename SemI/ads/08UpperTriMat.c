...
{
        system("clear");
        printf("08 Upper Triangular Matrix\n");
...
void getMatrix(unsigned int d, int matrix[d][d])
{
        ...
        for(rowNo = 0; rowNo < d; rowNo++)
        {
			...
			{
                        //we are in the lower half of upper triangular matrix
                                printf("0 at [%u][%u]\n", rowNo, colNo);
	...
	for(rowNo = 0; rowNo < d; rowNo++)
        {
                for(colNo = 0; colNo < d; colNo++)
                {
                        ...
                        //else we are in the lower half of upper tri matrix
                }
        }
        return;
}
