...
class UDPClient {
	...
	private void getFile() throws IOException, FileNotFoundException {
		...
		byte[] fileSizeBytes = ByteBuffer.allocate(4).putInt(fileSize).array();	//java.nio.*;
		DatagramPacket fileSizePacket = new DatagramPacket(fileSizeBytes, fileSizeBytes.length);
		contact.receive(fileSizePacket);
		fileSize = ByteBuffer.wrap(fileSizeBytes).getInt();
		System.out.println(" : readResponse : " + fileSize + "\n");

		//3rd packet - hopefully, 1 unfragmented packet of file
		byte[] file = new byte[fileSize];
		DatagramPacket filePacket = new DatagramPacket(file, fileSize);
